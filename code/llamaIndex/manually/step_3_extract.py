...
def run_extraction(extractor, nodes, cache_path, output_file_base):
    """Run the extraction and save the extracted results."""
    # Extract response to improve the extract percentage
    extractor.extract(
        nodes=nodes, 
        cache_path=cache_path,
        csv_file_name=output_file_base + '.csv'
...
def save_output_data(nodes, cache_path, output_file_base):
    """Save the nodes data to a JSONL file."""
    output_file = output_file_base + '.jsonl'
    save_nodes_jsonl(file_path=os.path.abspath(os.path.join(cache_path, output_file)), nodes=nodes)
...
def extract_pipeline():
    ...
    output_file_base = f'{input_file.split('.')[0]}_extract'
    ...
    # Save output data
    save_output_data(nodes, cache_path, output_file_base)
