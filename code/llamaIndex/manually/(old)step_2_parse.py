...
def submit_job(
    ...
    job_name = f'pid-{pid}_account-{account}_gpu-{gpu}_gn-{gn}_nnum-{node_number_per_process}'
    ...
    if job_name == None:
        return None, None
    save_file = generate_save_file_name(input_file, node_number_per_process, pid)
...
def generate_and_execute_slurm_script(
    ...
    if gpu == 'L40':
        return generate_and_execute_slurm_job(
            python_start_script=f"{python_file_name} --input_file {input_file} --action thread --pid {pid} --node_number_per_process {node_number_per_process}",
            account='guest',
            partition='guest-gpu',
            job_name={job_name},
            qos='low-gpu',
...
def generate_save_file_name(input_file, node_number_per_process, pid):
    """Generate a cache name based on input file, GPU, and node information."""
    return '_'.join(input_file.split('_')[:-1] + [f'nodeNum_{node_number_per_process}_pid_{pid}.jsonl'])
...
def resubmit_failed_job(script_path, job_dict, python_file_name, input_file, nodes_number):
    job_name, save_file_name = submit_job(
        ...
        python_file_name=python_file_name,
        input_file=input_file,
        node_number_per_process=int(job_dict['nnum']),
        nodes_numbers=nodes_number,
        account=job_dict['account']
    )
    return job_name, save_file_name

def handle_failed_job(job_name, save_file_path, script_path, is_restart, python_file_name, input_file, nodes_number):
    ...
    if not is_job_finished(save_file_path):
        ...
        if is_restart:
            return resubmit_failed_job(script_path, job_dict, python_file_name, input_file, nodes_number)
    return None, None
...
def monitor_and_restart(
    jobs, script_path, cache_path, python_file_name, input_file, nodes_number, check_interval=60, is_restart=True, 
):
    ...
    while len(jobs) > 0:
        ...
        for job_name, save_file in jobs:
            ...
            if is_job_finished(save_file_path):
                ...
                job_name, save_file = handle_failed_job(job_name, save_file_path, script_path, is_restart, python_file_name, input_file, nodes_number)
                if job_name != None:
...
def one_thread_parser(
    ...
    # Save nodes
    save_file_name = generate_save_file_name(input_file, node_number_per_process)
...
def merge(cache_path):
    prefix_file_name = "gpt-4o-batch-all-target_1_parser_ManuallyHierarchicalNodeParser_7652_nodeNum_50"
    ...
    cache_name = f"gpt-4o-batch-all-target_1_parser_ManuallyHierarchicalNodeParser_7652_nodeNum_{len(all_nodes)}.jsonl"
...
if __name__ == '__main__':
    ...
    gn = 1 # gpu number
    node_number_per_process=50 # node number for each process
    ...
    for file_name in os.listdir(cache_path):
        if file_name.startswith(basic_name) \
            and not file_name.endswith('processing.jsonl') \
            and not file_name.endswith('parser.jsonl'):
    ...
    args = load_args()
    if args.action == 'main':
        gpus = [(pid, 'V100') for pid in leave_tasks[:40]]
        jobs = []
